...
function OverviewTab({ myEvents, purchases }) {
  ...
  const totalTickets = myPurchases.reduce(
          ...
          <div className={styles.eventRowLeft}>
            ...
            <div>
              ...
              <div className={styles.eventRowMeta}>
                ...
                {e.tickets.map((t) => `${t.name}: ${t.sold}/${t.total}`).join(' · ')}
              </div>
            </div>
          </div>
          <button
            ...
            onClick={() => {
              if (window.confirm('Supprimer cet événement ?')) onDelete(e.id)
            }}
          >
            🗑️
          </button>
      ...
      <div className={styles.formRow}>
        <div className={styles.group} style={{ flex: 2 }}>
          ...
          <input
            ...
            placeholder="Ex: Festival de Jazz de Lomé"
            value={title}
      ...
      {ticketTypes.map((t, i) => (
        <div key={i} className={styles.ticketTypeRow}>
          ...
          {ticketTypes.length > 1 && (
            <button className={styles.removeTtBtn} onClick={() => removeTicketType(i)}>
              ✕
            </button>
          )}
        </div>
      ))}
      <button className={styles.addTtBtn} onClick={addTicketType}>
        + Ajouter un type
      </button>
...
function AttendeesTab({ myEvents, purchases, onCheckin }) {
  ...
  const attendees = purchases.filter((p) => p.items.some((i) => i.eventId === selectedId))
        ...
        <div className={styles.attendeeList}>
          {attendees.map((p) => {
            ...
            const eventCheckedIn =
              eventItems.length > 0 ? eventItems.every((i) => i.checkedIn) : false

            return (
              <div key={p.id} className={styles.attendeeRow}>
                <div className={styles.attendeeAvatar}>{p.userName[0].toUpperCase()}</div>
                ...
                <button
                  className={[styles.checkinBtn, eventCheckedIn ? styles.checkedIn : ''].join(' ')}
                  onClick={() => onCheckin(p.id, selectedId)}
                >
                  {eventCheckedIn ? '✓ Validé' : 'Check-in'}
                </button>
              </div>
            )
          })}
  ...
  return (
    <Modal open={open} onClose={onClose} size="lg">
      ...
      <ModalBody>
        ...
        {tab === 'attendees' && (
          <AttendeesTab myEvents={myEvents} purchases={purchases} onCheckin={onCheckin} />
        )}
      </ModalBody>
    </Modal>
  )
